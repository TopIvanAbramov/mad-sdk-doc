...
При загрузке рекламы необходимо передавать padId, который можно получить в рекламной админке либо от менеджера.
Для более точного таргетирования рекламы нужно передавать таргетинги в словаре targetings. Названия ключей должны совпадать между платформами.
...
Среди прочих в качестве аргументов методу `Mads#showInAppAd` можно передать `colors`, чтобы изменить цвета, используемые на диалоге с рекламой.
