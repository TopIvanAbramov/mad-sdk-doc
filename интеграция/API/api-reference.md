...
Создайте плейсмент в кабинете паблишера и получите `pad_id` - идентификатор рекламного слота. Подробнее: [Базовая инструкция для паблишеров](../../getting-started/publishers-guide.md).
...
```bash
curl -X GET "https://mads-serve.magnit.ru/v1/ads/get?pad_id={YOUR_PAD_ID}&user_id={USER_ID}"
```

**Обязательные параметры:**
- `pad_id` — идентификатор плейсмента
- `user_id` — идентификатор пользователя (для персонализации и статистики)
