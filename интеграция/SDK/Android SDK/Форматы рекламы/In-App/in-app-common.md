...
TODO: здесь не расрыты что за параметры и для чего нужны, так же что такое colors, есть замечание: стилизация показа состоит только из цветов и называется colors. Логичнее было бы иметь style, содержащий разные стили.

## 5. Реакция на события с рекламой

TODO: Здесь нужно расписать что в общем случае предоставляются такие колбеки для InApp рекламы в целом, но есть (если есть) специфичные рекламному формату - они должны быть описаны на странице конкретного формата.
